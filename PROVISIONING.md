# How to provision a new environment for calendar
...
* Enter domains to secure (for example): `mycalendar.com`
* Enter webroot path: `/home/deployer/apps/calendar/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/mycalendar.com/fullchain.pem /etc/ssl/calendar.crt
ln -s /etc/letsencrypt/live/mycalendar.com/privkey.pem /etc/ssl/calendar.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/calendar/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
2 6,18 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/calendar/shared/public/
```
